...
## Runtime Details
- Settings default to provider `openrouter` with model `deepseek/deepseek-v4-pro` (`packages/shared/src/settings.ts`). The provider implementation is OpenAI-compatible SSE streaming in `packages/infra/src/openai-compatible-provider.ts`.
